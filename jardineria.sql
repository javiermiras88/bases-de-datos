...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 05-12-2017 a las 09:54:52
...
INSERT INTO `cliente` (`CodigoCliente`, `NombreCliente`, `NombreContacto`, `ApellidoContacto`, `Telefono`, `Fax`, `LineaDireccion1`, `LineaDireccion2`, `Ciudad`, `Region`, `Pais`, `CodigoPostal`, `CodigoEmpleadoRepVentas`, `LimiteCredito`) VALUES
...
CREATE TABLE `detallepedidos` (
  `CodigoPedido` int(11) NOT NULL,
  `CodigoProducto` varchar(15) COLLATE utf8_unicode_ci NOT NULL
...
INSERT INTO `empleado` (`CodigoEmpleado`, `Nombre`, `Apellido1`, `Apellido2`, `Extension`, `Email`, `CodigoOficina`, `CodigoJefe`, `Puesto`) VALUES
...
--
-- Estructura de tabla para la tabla `gamasproducto`
--

CREATE TABLE `gamasproducto` (
...
--
-- Volcado de datos para la tabla `gamasproducto`
--

INSERT INTO `gamasproducto` (`Gama`, `DescripcionTexto`, `DescripcionHTML`, `Imagen`) VALUES
...
INSERT INTO `oficina` (`CodigoOficina`, `Ciudad`, `Pais`, `Region`, `CodigoPostal`, `Telefono`, `LineaDireccion1`, `LineaDireccion2`) VALUES
...
INSERT INTO `pago` (`CodigoCliente`, `FormaPago`, `IDTransaccion`, `FechaPago`, `Cantidad`) VALUES
...
INSERT INTO `pedido` (`CodigoPedido`, `FechaPedido`, `FechaEsperada`, `FechaEntrega`, `Estado`, `Comentarios`, `CodigoCliente`) VALUES
...
('AR-005', 'Mentha Sativa', 'Aromáticas', '15-20', 'Murcia Seasons', 'Â¿Quién no conoce la Hierbabuena? Se trata de una plantita muy aromática, agradable y cultivada extensamente por toda España. Es hierba perenne (por tanto vive varios años, no es anual). Puedes cultivarla en maceta o plantarla en la tierra del jardín o en un rincón del huerto. Lo más importante es que cuente con bastante agua. En primavera debes aportar fertilizantes minerales. Vive mejor en semisombra que a pleno sol.Si ves orugas o los agujeros en hojas consecuencia de su ataque, retíralas una a una a mano; no uses insecticidas químicos.', 140, '1.00', '0.00', 'menta_sativa.jpg'),
...
('FR-10', 'Limonero 2 años injerto', 'Frutales', '', 'NaranjasValencianas.com', 'El limonero, pertenece al grupo de los cítricos, teniendo su origen hace unos 20 millones de años en el sudeste asiático. Fue introducido por los árabes en el área mediterránea entre los años 1.000 a 1.200, habiendo experimentando numerosas modificaciones debidas tanto a la selección natural mediante hibridaciones espontáneas como a las producidas por el hombre, en este caso buscando las necesidades del mercado.', 15, '7.00', '5.00', 'injerto_limonero.jpg'),
('FR-100', 'Nectarina', 'Frutales', '8/10', 'Frutales Talavera S.A', 'Se trata de un árbol derivado por mutación de los melocotoneros comunes, y los únicos caracteres diferenciales son la ausencia de tomentosidad en la piel del fruto. La planta, si se deja crecer libremente, adopta un porte globoso con unas dimensiones medias de 4-6 metros', 50, '11.00', '8.00', 'nectarina.jpg'),
...
('OR-001', 'Arbustos Mix Maceta', 'Ornamentales', '40-60', 'Valencia Garden Service', '', 25, '5.00', '4.00', 'maceta.jpg'),
...
ALTER TABLE `detallepedidos`
  ADD PRIMARY KEY (`CodigoPedido`,`CodigoProducto`),
  ADD KEY `IDX_F2FDC88E29A7591` (`CodigoPedido`),
  ADD KEY `IDX_F2FDC88E4E23B11A` (`CodigoProducto`);
...
--
-- Indices de la tabla `gamasproducto`
--
ALTER TABLE `gamasproducto`
...
--
ALTER TABLE `cliente`
  ADD CONSTRAINT `Clientes_EmpleadosFK` FOREIGN KEY (`CodigoEmpleadoRepVentas`) REFERENCES `empleado` (`CodigoEmpleado`);

--
-- Filtros para la tabla `detallepedido`
--
ALTER TABLE `detallepedido`
  ADD CONSTRAINT `DetallePedidos_PedidoFK` FOREIGN KEY (`CodigoPedido`) REFERENCES `pedido` (`CodigoPedido`),
  ADD CONSTRAINT `DetallePedidos_ProductoFK` FOREIGN KEY (`CodigoProducto`) REFERENCES `producto` (`CodigoProducto`);

--
-- Filtros para la tabla `detallepedidos`
--
ALTER TABLE `detallepedidos`
  ADD CONSTRAINT `FK_F2FDC88E29A7591` FOREIGN KEY (`CodigoPedido`) REFERENCES `pedido` (`CodigoPedido`);

--
-- Filtros para la tabla `empleado`
--
ALTER TABLE `empleado`
  ADD CONSTRAINT `Empleados_EmpleadosFK` FOREIGN KEY (`CodigoJefe`) REFERENCES `empleado` (`CodigoEmpleado`),
  ADD CONSTRAINT `Empleados_OficinasFK` FOREIGN KEY (`CodigoOficina`) REFERENCES `oficina` (`CodigoOficina`);

--
-- Filtros para la tabla `pago`
--
ALTER TABLE `pago`
  ADD CONSTRAINT `Pagos_clienteFK` FOREIGN KEY (`CodigoCliente`) REFERENCES `cliente` (`CodigoCliente`);

--
-- Filtros para la tabla `pedido`
--
ALTER TABLE `pedido`
  ADD CONSTRAINT `Pedidos_Cliente` FOREIGN KEY (`CodigoCliente`) REFERENCES `cliente` (`CodigoCliente`);

--
-- Filtros para la tabla `producto`
--
ALTER TABLE `producto`
  ADD CONSTRAINT `Productos_gamaFK` FOREIGN KEY (`Gama`) REFERENCES `gamasproducto` (`Gama`);
COMMIT;
